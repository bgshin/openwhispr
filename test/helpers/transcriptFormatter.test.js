...
test("language-specific export selects Korean and English independently of spoken language", () => {
  const segments = [
    { text: "안녕하세요", translatedText: "Hello" },
    { text: "Good morning", translatedText: "좋은 아침입니다" },
  ];

  assert.deepEqual(
    selectTranscriptLanguage(segments, "ko").map((segment) => segment.text),
    ["안녕하세요", "좋은 아침입니다"]
  );
  assert.deepEqual(
    selectTranscriptLanguage(segments, "en").map((segment) => segment.text),
    ["Hello", "Good morning"]
  );
});
