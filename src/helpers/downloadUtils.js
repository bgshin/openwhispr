...
async function checkDiskSpace(directory, requiredBytes) {
  try {
    ...
    // Disk-space inspection is an optional preflight: unsupported filesystems
    // must not block a download. The subsequent write/extraction still fails
    // normally and reports the filesystem error if capacity is insufficient.
