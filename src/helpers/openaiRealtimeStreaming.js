...
class OpenAIRealtimeStreaming {
  ...
  async connect(options = {}) {
    const { apiKey, model, preconfigured, inputRate, captureRate, language, createSocket } =
      options;
      ...
      switch (event.type) {
                ...
                session: {
                  ...
                  audio: {
                    input: {
                      ...
                      transcription: {
                        model: this.model,
                        // The meeting recorder already resolved the user's STT
                        // language. Sending it avoids unreliable auto-detection
                        // for short Korean utterances.
                        ...(this.language ? { language: this.language } : {}),
                      },
                      turn_detection: {
                        type: "server_vad",
                        // Meeting microphones often run without browser AGC
                        // (to keep diarization input faithful). 0.6 misses
                        // normal conversational Korean/English levels.
                        threshold: 0.35,
        ...
        case "conversation.item.input_audio_transcription.completed": {
          ...
          if (transcript) {
            ...
            debugLogger.info("OpenAI Realtime transcript received", {
        ...
        case "conversation.item.input_audio_transcription.failed": {
          ...
          const error = new Error(detail.message || "OpenAI could not transcribe this audio.");
          error.code = detail.code;
          debugLogger.error("OpenAI Realtime transcription failed", {
            code: detail.code,
  ...
  _markConnected() {
    ...
    debugLogger.info("OpenAI Realtime transcription session ready", {
      model: this.model,
