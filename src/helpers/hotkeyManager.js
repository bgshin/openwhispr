...
class HotkeyManager extends EventEmitter {
  ...
  async getSavedHotkey() {
    ...
    // A temporary registration fallback must not overwrite the user's saved
    // preference; retry it on each startup and fall back only while its
    // registration conflict remains.
    if (this.mainWindow && !this.mainWindow.isDestroyed()) {
      try {
