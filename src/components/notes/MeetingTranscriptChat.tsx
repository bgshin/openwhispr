...
import { isKoreanText } from "../../helpers/meetingBilingualTranslation";
import {
...
export function MeetingTranscriptChat({
  ...
  return (
    <div className="h-full relative">
      ...
      >
        <div className="grid grid-cols-2 gap-3 sticky top-0 z-10 bg-background/95 py-2 text-xs font-medium text-muted-foreground backdrop-blur">
          <span>한국어</span>
          <span>English</span>
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="relative w-full max-w-[960px]">
                ...
                >
                  {[koreanText, englishText].map((text, column) => (
                    <div
                      key={column}
                      className={cn(
                        "min-h-9 px-3 py-1.5 rounded-lg text-[13px] leading-relaxed",
                        selfSide
                          ? "bg-primary/90 text-primary-foreground"
                          : "bg-surface-2 border border-border/30 text-foreground",
                        isSystemSpeaker &&
                          column === 0 &&
                          cn("border-l-2", SPEAKER_BORDER_COLORS[colorIdx]),
