...
interface NoteEditorProps {
  ...
  onExportTranscript?: (format: "txt" | "srt" | "json" | "md", language: "ko" | "en") => void;
...
export default function NoteEditor({
  ...
  }, [displaySegments, isRecording, persistDisplaySegments]);
                        ...
                        {(["ko", "en"] as const).map((language) => (
                          <DropdownMenuSub key={language}>
                            ...
                            <DropdownMenuSubContent sideOffset={4}>
                              {(
                                [
                                  ["txt", t("notes.editor.asTranscriptText")],
                                  ["srt", t("notes.editor.asSubtitles")],
                                  ["md", t("notes.editor.asTranscriptMarkdown")],
                                  ["json", t("notes.editor.asJson")],
                                ] as const
                              ).map(([format, label]) => (
                                <DropdownMenuItem
                                  key={format}
                                  onClick={() => onExportTranscript(format, language)}
