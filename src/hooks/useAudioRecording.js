...
export const useAudioRecording = (toast, options = {}) => {
  ...
  const shouldFetchSttConfig = useCallback(() => {
  ...
  const performStartRecording = useCallback(
    async ({ voiceAgentRequested = false, translationRequested = false } = {}) => {
      ...
      try {
        ...
        // Retry cloud STT config fetch if it wasn't loaded on mount (e.g. auth
        // wasn't ready). Local/BYOK/self-hosted modes do not need the cloud
        // endpoint and should stay quiet when no API URL is configured.
        if (shouldFetchSttConfig() && !audioManagerRef.current.sttConfig) {
  ...
  }, [toast, onToggle, performStartRecording, performStopRecording, shouldFetchSttConfig, t]);
