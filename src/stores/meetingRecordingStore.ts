...
async function translateMeetingSegmentNow(segment: TranscriptSegment): Promise<string | null> {
  ...
  const isSelfHosted =
    settings.translationMode === "self-hosted" && !!settings.translationRemoteUrl;
  const provider = isCloud ? "openwhispr" : settings.translationProvider || undefined;
...
const getSpeakerNumericIndex = (speakerId?: string): number | null => {
  ...
  const match = speakerId.match(/(?:mic_)?speaker_(\d+)/);
...
const isSegmentWithinIdentificationWindow = (
  ...
  if (segment.timestamp == null) return false;
  return (
...
const getMeetingTranscriptionOptions = () => {
  ...
  // Meetings may switch between Korean and English within a single turn. Do
  // not inherit the dictation language (for example, "en") and force every
  // speaker through it; supported streaming providers auto-detect per turn.
  const language = undefined;
  const keyterms = (state.customDictionary ?? []).filter(Boolean);

  if (resolved.useLocalWhisper) {
    // Parakeet is not registered as a Korean-capable local engine. Prefer
    // Whisper instead of silently running Korean meeting audio through an
    // unsupported recognizer.
    const useWhisperForKorean =
      language === "ko" && resolved.localTranscriptionProvider === "nvidia";
    if (useWhisperForKorean) {
      logger.warn(
        "Korean meeting transcription requires Whisper; overriding the selected Parakeet provider",
        {},
        "meeting"
      );
    }
    return {
      ...
      localProvider: useWhisperForKorean ? "whisper" : resolved.localTranscriptionProvider,
      localModel:
        !useWhisperForKorean && resolved.localTranscriptionProvider === "nvidia"
  ...
  return { provider: `${provider.id}-realtime` as const, model, mode, language, keyterms };
};
...
function applySpeakerIdentification(
  ...
  if (
    (identification.source && segment.source !== identification.source) ||
...
function assignProvisionalSpeaker(segment: TranscriptSegment): TranscriptSegment {
  ...
  if (segment.source === "mic") {
    const previousMicSegment = [...segmentsRefValue]
      .reverse()
      .find(
        (candidate) =>
          candidate.source === "mic" &&
          candidate.speaker &&
    ...
    return normalizeTranscriptSegment({
      ...segment,
      speaker: previousMicSegment?.speaker || "mic_speaker_0",
      speakerName: previousMicSegment?.speakerName,
  ...
  try {
    ...
    const segmentCleanup = window.electronAPI?.onMeetingTranscriptionSegment?.(
      (data: {
        ...
        queueMeetingSegmentTranslation(seg);
